...
function SobremesaSelecionada(botao) {
  const botaoAnterior = document.querySelector(
    ".caixa-de-sobremesas .selecionado"
  );
  if (botaoAnterior !== null) {
    botaoAnterior.classList.remove("selecionado");
  }
  ...
  valorSobremesaErrado = valorSobremesaErrado
    .replace("R$", "")
    .replace(",", ".");
...
function VerificaSelecao() {
  ...
  const verificaBebida = document.querySelector(
    ".caixa-de-bebidas .selecionado"
  );
  const verificaSobremesa = document.querySelector(
    ".caixa-de-sobremesas .selecionado"
  );
